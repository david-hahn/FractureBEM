...
#include <openvdb/tools/MeshToVolume.h> // needed only for vdb::util::INVALID_IDX
    ...
    int SubsampledCrackTip::buildMeshUpdate(
        ...
        for(elem_map::const_iterator it = crackTips.begin(); it!= crackTips.end(); ++it){
			...
			if(collapsed){ //just add the triangle connecting the old crack-tip edge to the point it has been collapsed into
				...
				//newCrackTips[nextCtElem][2]=nd_a; // old version for quad-based state check
				//newCrackTips[nextCtElem][3]=nd_b;
...
//        }        
	...
	int SubsampledCrackTip::startCrack(
		...
		node_map addNodes;
        elem_map addElems, addCrackTips, addParents;
        id_map addRegions;
		state_map addStates;
        ...
        /**/
        // update level-set (could start the level-set with a higher-resolved half-disc?)
    ...
    int SubsampledCrackTip::vtkWrite(std::string fileName){
        id_map e1; id_set e2; elem_map e3; state_map e4;
        ...
        PostProcessor ctOut(ctNodes, ctElems, e1, e2, e3,e3 , e4);
    ...
    bool SubsampledCrackTip::checkSubdivideEdge(
        ...
        if( !reject && //(newA!=-1 && newB!=-1) &&
            ...
            if(newB!=-1){ // add tri (b,c,newB)
                ...
                /*nextElem=*/addTri( // nextElem must be updated in buildMeshUpdate later
