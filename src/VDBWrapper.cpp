	...
	bool VDBWrapper::lineCrackSelfIntersection(const Vec3d& a, const Vec3d& b, int grid){
		...
		while( d*d < lenSqr && !found){
			...
			if( tmp< (-FLT_EPSILON) && val > (-halfDiag*voxelSize) ){ // near a crack
				...
				if( dir > /*0.1*voxelSize /*/FLT_EPSILON/**/ ){ // trigger only when moving towards a surface, as the self-surface will be part of the grid, and the crack-tip goes away from it
					found=true;
