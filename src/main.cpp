...
int main(int argc, char* argv[]){
	...
	try {
		...
		printf("%% ... input files are \"%s.*\"\n", inpFile.getValue().c_str());
		printf("%% ... material parameters (E,nu) are (%.2le, %.2lf)\n", youngsMod.getValue(), poissonsRatio.getValue());
		if(is2D.getValue()) printf("%% ... computing only 2d displacements in x-y-plane\n");
		if(outFile.isSet()) printf("%% ... output files are \"%s_*.vtk/.vdb\"\n", outFile.getValue().c_str());
		t1=omp_get_wtime();
		printf("%% ... %.4lfs CLI parsed\n",t1-t0);
		t0=t1;

		printf("%% initializing simulation ... \n");
		FractureBEM sim(resMesh.getValue()); //resolution of crack-meshes (crack-tips will be subdivided, also "time-step" of propagation is chosen to match this)
		...
		printf("%% reading model ...      ");
		{
			...
			t1=omp_get_wtime();
			printf("\t%.4lfs - have %d elements and %d nodes\n",t1-t0, sim.getElems().size() ,sim.getNodes().size());
			t0=t1;
		}
		...
		if(remeshTris.isSet()){ // do remeshing based on the level-set representation of the initial model to build the BEM model
			...
			printf("%% initializing mesh regions from file %s.regions ...\n",inpFile.getValue().c_str());
            ...
            if(remeshTris.getValue()>3){ // remeshing to the desired resolution
                ...
                regionHandler.assignRegions(sim.getRegions(), sim.getNodes(),sim.getElems());
                t1=omp_get_wtime();
                printf("\t%.4lfs - have %d elements\n",t1-t0,sim.getElems().size());
                t0=t1;
            }

            printf("%% initializing BEM solver ... ");
            sim.initBEM(*material, is2D.getValue(), bcStrings.getValue());
		...
		printf("%% initializing fracture model ... \n");
		sim.initFractureModel(*material, outVDBsubstep.isSet());
		...
		for(k=0; k<=steps && doFracture; ++k){
			printf("%% computing displacements ... ");
			sim.computeBEM();
			t1=omp_get_wtime();
			printf("\t%.4lfs\n",t1-t0);
			t0=t1;
			...
			if( outFile.isSet() ){
                ...
                sim.writeCrackTip(fileName.str()+"_ct.vtk");

				t1=omp_get_wtime();
				printf("\t%.4lfs (%s)\n",t1-t0, fileName.str().c_str());
				t0=t1;
			}
			...
			if(k<steps && doFracture){
				printf("%% simulating fractures ... ");
				//ToDo: how to check whether the simulation should terminate --> on first segmentation?
				...
				if(newCracks>0) printf(", just added %d new fractures\n", newCracks);
				else printf("\n");
				t0=t1;
			}
		}
        if( outFile.isSet() ){ // write final state
            ...
            printf("%% writing final VTK & VDB output ... ");
            stringstream fileName;
            fileName << outFile.getValue() << "_";
            fileName.fill('0'); fileName.width(4);
            fileName << k;
